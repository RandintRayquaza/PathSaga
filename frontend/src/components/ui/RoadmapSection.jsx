  ...
  return (
    <Card className="col-span-full lg:col-span-2">
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <p className="text-xs text-zinc-500">{t('dashboard.your_plan')}</p>
          <h2 className="font-semibold text-zinc-100">{t('dashboard.roadmap_title')}</h2>
        </div>
      </div>

      {roadmap.length === 0 ? (
        <p className="text-sm text-zinc-500 text-center py-6">{t('dashboard.no_roadmap')}</p>
      ) : (
        <div className="space-y-6">
          {roadmap.map(({ phase, milestones }) => (
            <div key={phase}>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-violet-400/70 mb-3">{t(phase)}</h3>
